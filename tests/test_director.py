...
from ai_video_factory.director import VideoDirector, _validate_script_against_rules
from ai_video_factory.pipeline import PipelineError


class TestValidateScriptAgainstRules:
    def test_passes_valid_script(self):
        violations = _validate_script_against_rules("Lost forever.\nThis changed everything.\nAnd nobody saw it coming.", hook="Lost forever")
        assert violations == []
    ...
    def test_fails_hook_too_long(self):
        hook = "This is a very long hook that should not be allowed."
        violations = _validate_script_against_rules(hook + "\nLine two.\nLine three.", hook=hook)
...
class TestBuildScript:
    def test_template_fallback_when_no_model_key(self):
        ...
        director.creative_brief = {"topic": "Test Topic", "hook": "The Real Reason", "strongest_angle": "A wild twist", "emotion": "dramatic"}
    ...
    def test_llm_path_parses_json_and_validates(self):
        ...
        director.creative_brief = {"topic": "Minecraft", "hook": "Lost forever", "strongest_angle": "Betrayal", "emotion": "dramatic"}
        ...
        assert _validate_script_against_rules(script, "Lost forever") == []
    ...
    def test_llm_fallback_to_template_on_rule_violation(self):
        ...
        with patch("ai_video_factory.director.call_model", return_value=json.dumps({"lines": bad_lines})):
            director.build_script({})
        assert director.creative_brief["script_source"] == "template"

    def test_markdown_fences_stripped(self):
        director = VideoDirector(model_key="fake-key")
        director.creative_brief = {"topic": "X", "hook": "Hook", "strongest_angle": "Angle", "emotion": "dramatic"}
        director._target_seconds = 45.0
...
class TestDirectorProduceErrorHandling:
    ...
    def test_produce_aborts_on_critical_research_failure(self, mock_deep, mock_research):
        ...
        with pytest.raises(PipelineError) as exc_info:
            VideoDirector().produce("Minecraft", target_seconds=45.0)
    ...
    def test_produce_aborts_on_critical_package_failure(self, mock_deep, mock_research, mock_pkg):
        mock_deep.return_value = {}
        mock_research.return_value = {"topic": "Minecraft", "who_what": "stuff", "main_conflict": "betrayal", "emotion": "dramatic", "strongest_angle": "twist"}
