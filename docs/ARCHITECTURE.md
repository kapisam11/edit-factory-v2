...
under `archive/` is not part of the supported runtime path. The canonical installed command is
`aivf` -> `cli.py`; `cli_v2.py` is deprecated compatibility code and should not receive new
features.
